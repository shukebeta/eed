...
@test "performance: comparison - old vs new method (500 lines)" {
    ...
    start_time=$(date +%s.%N)
    old_result=$(process_and_fix_script_old "$script" 2>/dev/null)
    end_time=$(date +%s.%N)
    old_duration=$(echo "$end_time - $start_time" | bc -l 2>/dev/null || echo "5.0")
    
    # Test new method (function-based)  
    start_time=$(date +%s.%N)
    new_result=$(process_and_fix_script "$script" 2>/dev/null)
    end_time=$(date +%s.%N)
    ...
    # New method should be faster (or at least not slower)
    [ "$(echo "$new_duration <= $old_duration" | bc -l 2>/dev/null || echo 1)" -eq 1 ]
}